    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5,min_tracking_confidence=0.5) as holistic:
        while True:
            ...
            if results_holistic.pose_landmarks:
                mpDraw.draw_landmarks(img, results_holistic.pose_landmarks, mp_holistic.POSE_CONNECTIONS)
            ...
            if results_holistic.pose_landmarks:
                ...
                cv2.line(img, (left_x,left_y), (right_x,right_y), (0, 0, 255), 2)
                mid_x = left_x + int(abs(right_x - left_x) / 2)
                mid_y = int(abs(right_y + left_y) / 2)
                cv2.circle(img, (mid_x, mid_y), 2, (255, 255, 255), 2)
                if rec != None:
